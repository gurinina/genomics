...
g=ggplot(data = surveys_complete, mapping = aes(x = species_id, y = weight)) +
  geom_boxplot(alpha = 0) +
  geom_jitter(alpha = 0.3, color = "tomato")
...
ggplot(data = surveys_complete, mapping = aes(x = species_id, y = hindfoot_length,col=factor(plot_id))) +
  ...
  geom_boxplot(alpha = 0) 
```
Because plot_id is continuous and should be a factor.
...
My contribution:
```{r gg}
 g<- ggplot(data = yearly_sex_counts,
                  aes(x = year, y = n, color = sex)) +
    geom_line() +
    facet_wrap(vars(genus)) +
    labs(title = "Observed genera through time",
    ...
    g <- g + theme(strip.background = element_rect(fill="dodgerblue"),
