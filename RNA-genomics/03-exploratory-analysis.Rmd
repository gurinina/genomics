...
packages <- c("DESeq2" ,"devtools", "dplyr","edgeR",               
...
bam.files = gsub("BAM/","BAM/RBAM/",bam.files)
...
output_file = bam,useAnnotation=TRUE,annot.ext = "FASTQ/r64/Saccharomyces_cerevisiae.R64-1-1.96.gtf.gz",isGTF = TRUE,nthreads=64,sortReadsByCoordinates=TRUE, keepReadOrder=FALSE,
...
fc <- featureCounts(bam, annot.ext=gtf.file, 
isGTFAnnotationFile=TRUE,
isPaired=FALSE, fraction = TRUE,
###################
...
mse = assays(se)$counts
g=grep("^Y" ,rownames(mse))
mse = mse[g,]
mse = mse[order(rownames(mse)),]
```
...
Make a dataframe with ORF and GENE as columns and arrange the data in order of padj and descending order by absolute value of log2FoldChange. Filter the results for those records that have padj < 0.05 and absolute value of res$log2FoldChange > 2 (i.e., at least double or half the level of expression).
```{r}
# To make a data.frame type:
# 
dres = data.frame(res,stringsAsFactors = F,orf = rownames(res))
m = match(dres$orf,names(gene))
# check to see that all orfs were found in the names of gene
table(is.na(m)) # is Not Available (missing)
dres$gene = gene[m]


dres = dres %>% arrange(padj,  desc(abs(log2FoldChange)))
...
with or without shrinkage. Make the same data.frame as the one above
...
Question: look up a gene with an intron. How do the reads compare to genes without introns?
